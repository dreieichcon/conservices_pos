...
public class TransactionRepository(IDbService<ClientConfig> clientConfigService) : AbstractServerRepository(clientConfigService), ITransactionRepository
{
	public async Task<TransactionReceipt?> CommitTransaction(ClientTransaction transaction)
	{
		...
		return DeserializeOrNull<TransactionReceipt>(result);
	}
}